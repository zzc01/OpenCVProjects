...
void addNewBlob(const Blob& currentFrameBlob, std::vector<Blob>& existingBlobs);
...
int main()
{
	...
	while (capVideo.isOpened() && CheckEscKey != 27)
	{
		...
		// draw and show contours function to include the following three operations  
		...
		for (const auto& convexHull : convexHulls)
		{
			...
			if (
				possibleBlob.currentBoundingRect.area() > 100 &&
				...
				(cv::contourArea(possibleBlob.currentContour) / (double)possibleBlob.currentBoundingRect.area() > 0.4)
				)
			{
